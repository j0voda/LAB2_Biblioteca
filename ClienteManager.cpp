...
Cliente ClienteManager::crearCliente(){
    ...
    Membresia membresia = _membresiaManager.asignarMembresia();
...
void ClienteManager::listarClientes() {
    ...
    for (const Usuario &cliente : clientes) {
        Cliente c = Cliente(cliente.getNombre(), cliente.getApellido(), cliente.getTelefono(), cliente.getMail(), cliente.getId(), cliente.getClave(), 2, c.getMembresia());
...
void ClienteManager::listarClientesPorApellido() {
    ...
    cout << "Clientes por apellido: " << endl;
    for (const Usuario &cliente : clientes) {
        Cliente c = Cliente(cliente.getNombre(), cliente.getApellido(), cliente.getTelefono(), cliente.getMail(), cliente.getId(), cliente.getClave(), 2, c.getMembresia());
        this->mostrarCliente(c);
    }
