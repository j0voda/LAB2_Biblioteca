...
void LibroManager::listarLibrosPorNombre() {
	...
	sort(libros.begin(), libros.end(), [](const Libro &a, const Libro &b) {
        string tituloA = a.getTitulo();
        string tituloB = b.getTitulo();
		transform(tituloA.begin(), tituloA.end(), tituloA.begin(), ::tolower);
        transform(tituloB.begin(), tituloB.end(), tituloB.begin(), ::tolower);

        return tituloA < tituloB;
    });
    cout << "Libros por titulo: " << endl;
    cout<<endl;
    for (const Libro &libro : libros) {
        Libro l = Libro(libro.getIdLibro(),libro.getTitulo(),libro.getAutor(),libro.getCategoria(), libro.getAnioPublicacion(),libro.getEjemplares(),libro.getDisponibles(),libro.getEstado());
        this->mostrarLibro(l);
    }
}
...
void LibroManager::listarLibrosPorCategoria() {
    vector<Libro> libros = _libroArchivo.leerTodos();
	sort(libros.begin(), libros.end(), [](const Libro &a, const Libro &b) {
        string catA = a.getCategoria().getNombre();
        string catB = b.getCategoria().getNombre();
		transform(catA.begin(), catA.end(), catA.begin(), ::tolower);
        transform(catB.begin(), catB.end(), catB.begin(), ::tolower);

        return catA < catB;
    });
    cout << "Libros por categoria: " << endl;
    cout<<endl;
    for (const Libro &libro : libros) {
        Libro l = Libro(libro.getIdLibro(),libro.getTitulo(),libro.getAutor(),libro.getCategoria(), libro.getAnioPublicacion(),libro.getEjemplares(),libro.getDisponibles(),libro.getEstado());
        this->mostrarLibro(l);
...
void LibroManager::listarLibrosPorAutor() {
    vector<Libro> libros = _libroArchivo.leerTodos();
    sort(libros.begin(), libros.end(), [](const Libro &a, const Libro &b) {
        string autorA = a.getAutor().getNombre();
        string autorB = b.getAutor().getNombre();
		transform(autorA.begin(), autorA.end(), autorA.begin(), ::tolower);
        transform(autorB.begin(), autorB.end(), autorB.begin(), ::tolower);
